...
class ExpanderConstructor:
    ...
    def __init__(self, qtd_of_col : int, type_of_plot: str, content: any, label: str, container_size: bool = default_container_size) -> None:
        self.qtd_of_col: int = qtd_of_col
        self.expander: DeltaGenerator = st.expander(label)
        self.content: any = content
        self.type_of_plot: str = type_of_plot
        self.expander_size: str = container_size
        self._plot_is_valid: bool = self.__validte_plot()
    
    def build_expander(self) -> None:
    ...
    def __check_columns_size(self, columns: DeltaGenerator) -> Optional[str]:
        return 'multiple_columns' if len(columns) > 1 else None
    
    def __plot_in_multiple_columns(self, columns: DeltaGenerator, content, container_width: str) -> None:
    ...
    def __plot_in_solo_column(self, content, container_width) -> None:
