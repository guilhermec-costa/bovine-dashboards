...
def last_battery(data):
    ...
    fig.update_layout(title=dict(text=f'Last Battery Indicator until {(datetime.now() + timedelta(hours=-3)).strftime("%Y/%m/%d %H:%M:%S")}', x=0.5, y=0.93, yanchor='top', xanchor='center',
                                 font=dict(size=18)))
