...
def messages_a_day(data, date_period):
    fig = px.bar(data_frame=data, x='PLM', y='Sent Messages', color='Sent Messages', color_continuous_scale=px.colors.sequential.Plotly3_r, height=550,
                 text_auto=True)
    ...
    fig.update_layout(title=dict(text=f'Sent messages from {start_year}/{start_month}/{start_day} to {end_year}/{end_month}/{end_day}', 
                                 font=dict(size=25), x=0.5, y=0.96, yanchor='top', xanchor='center'))
